...
export class AuthService {
  ...
  async verifyUser(grant_type: string, username: string, password: string): Promise<AuthInfo> {
    const config = {
      headers: {
    ...
    await instance.post(encodeURI(`oauth/token?grant_type=${grant_type}&username=${username}&password=${password}`).replace(/\+/g, '%2B'), {}, config)
      .then(async response => {
