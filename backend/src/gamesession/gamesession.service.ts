...
export class GameSessionService {
    ...
    async createSession(access_token: string, creator?: string, game?: string, savegame?: string): Promise<string> {
        let data = null;
        ...
        return instance.post(encodeURI(`?access_token=${access_token}`).replace(/\+/g, '%2B'), data).then(response => {
    ...
    async joinSession(session_id: string, name: string, access_token: string): Promise<string> {
        return instance.put(encodeURI(`${session_id}/players/${name}?access_token=${access_token}`).replace(/\+/g, '%2B'))
        .then(response => {
            return response.data as string
        });
    }

    async exitSession(session_id: string, name: string, access_token: string): Promise<string> {
        return instance.delete(encodeURI(`${session_id}/players/${name}?access_token=${access_token}`).replace(/\+/g, '%2B'))
        .then(response => {
            return response.data as string
        });
    }
